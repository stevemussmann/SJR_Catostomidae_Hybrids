# Bluehead Sucker x Flannelmouth Sucker entropy
...
1. Run `twospecies.sh` which uses the `Cdis_x_Clat_map.txt` file to pull only these two species from the filtered VCF file produced during running admixpipe
